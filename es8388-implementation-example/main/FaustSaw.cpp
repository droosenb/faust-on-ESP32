...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("filename", "FaustSaw.dsp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.3");
		m->declare("name", "FaustSaw");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("FaustSaw");
		ui_interface->addHorizontalSlider("freq", &fHslider0, 440.0f, 20.0f, 2000.0f, 0.00999999978f);
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp0 = std::max<float>(1.00000001e-07f, std::fabs(fRec2[0]));
...
FaustSaw::FaustSaw(int sample_rate, int buffer_size)
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void FaustSaw::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float FaustSaw::getParamValue(const std::string& path)
{
...
extern "C" void app_main()
{
    ...
    FaustSaw* DSP = new FaustSaw(48000, 32);
    DSP->start();
