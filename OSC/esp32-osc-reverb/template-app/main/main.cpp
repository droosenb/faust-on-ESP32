            ...
            // Data received
                ...
                while (pr.isOk() && (msg = pr.popMessage()) != 0) {
                    ...
                    if (msg->match(msgAddress).popFloat(floatArg).isOkNoMoreArgs()) {
                        ...
                        verb->setParamValue(msgAddress, floatArg);
...
void app_main(void)
{
    ...
    config.adc_input = AUDIO_HAL_ADC_INPUT_LINE2; //set the codec input configuration. line
    ...
    config.i2s_iface.samples = AUDIO_HAL_24K_SAMPLES; //set the sample rate (must match faust)
    ...
    /* control functions */
    codec.es8388_set_voice_volume(80 ); //set the voice volume to 100

    codec.es8388_set_voice_mute(false); //set the voices to be unmuted (unmuted by default)

    codec.es8388_set_mic_gain(MIC_GAIN_0DB); //set the mic gain to be line level

    
    
    /* Simple faust code */
    verb = new reverb(24000, 32);
    verb->start(); 

    xTaskCreatePinnedToCore(osc_server_task, "OSC_server", 4096, NULL, 5, NULL, 0);
