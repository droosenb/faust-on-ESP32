...
static const char *TAG = "OSC Device";
...
static void udp_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        //initialize sending socket. should maybe try to do this with one socket. maybe funcitonal? idk enough about udp sockets
        int sock_send = socket(addr_family, SOCK_DGRAM, ip_protocol);
        if (sock_send < 0) {
            ESP_LOGE(TAG, "Unable to create socket: errno %d", errno);
            break;
        }
        ESP_LOGI(TAG, "Socket created, sending to %d", OUTPORT);
            ...
            // Data received
                ...
                while (pr.isOk() && (msg = pr.popMessage()) != 0) {
                    ...
                    // "hello" message
                        ...
                        //print our hello to the chat
                        //need to fix ip address issue. now just outputs worthless
                        printf("address: %s, in:%i, out:%i\n", "ADD IP ADDRESS", PORT, OUTPORT);
                        
                        oscpkt::Message s_msg("/hello"); //just an example message
                        s_msg.pushStr("ADD IP ADDRESS"); //add our address
